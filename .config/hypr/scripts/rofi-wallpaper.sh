...
if pgrep -x qmlscene >/dev/null || pgrep -x qml6 >/dev/null || pgrep -x rofi >/dev/null || pgrep -x wofi >/dev/null; then
    pkill -x qmlscene 2>/dev/null || true
    pkill -x qml6 2>/dev/null || true
